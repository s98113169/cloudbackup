...
class Records extends CI_Controller {
	...
	public function insertRecords()
	{
		...
		$this->form_validation->set_rules('company_name', '公司名稱', 'required|max_length[20]');
		$this->form_validation->set_rules('department', '部門', 'required|max_length[20]');
		$this->form_validation->set_rules('employees', '公司規模', 'required');
		$this->form_validation->set_rules('title', '職稱', 'required|max_length[20]');
		$this->form_validation->set_rules('telephone', '電話', 'required|max_length[20]');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				...
				'question5' =>  json_encode($this->input->post('question5')),
